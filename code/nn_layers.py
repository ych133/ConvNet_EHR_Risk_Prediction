...
class CopiedLayer(Layer):
  def __init__(self, layer=None, name=None):
    super(CopiedLayer, self).__init__()
    if not name:
      if hasattr(layer, 'name'):
        name = layer.name + '_copied'
      else:
        name = layer.__class__.__name__ + '_copied'
    self.name = name    
    self.layer = layer

  def output_func(self, input):
    return self.layer.output_func(input)
    
  def __repr__(self):
    layer_str = '\n'.join(['\t{}'.format(line) for line in str(self.layer).splitlines()])

    return "{}\n{}".format(self.name, layer_str)    
...
class SumMergeLayer(FeedForwardNet):
    def output_func(self, input):
        self.layers[0].set_input(input)
        this_out = self.layers[0].output
        for layer in self.layers[1:]:
            layer.set_input(input)
            this_out = this_out + layer.output
        return this_out

class MultiplyMergeLayer(FeedForwardNet):
    def output_func(self, input):
        self.layers[0].set_input(input[0])
        this_out = self.layers[0].output    # [n_batch, n_kernel, T, n_dim]
        self.layers[1].set_input(input[1])
        time_coef = self.layers[1].output    # [n_batch, T]
        return this_out * time_coef.dimshuffle([0, 'x', 1, 'x'])

class TimeShiftLayer(FeedForwardNet):
    def __init__(self, shift=0, border=0, exp_coef=0.01, time_term='delta'):
        super(TimeShiftLayer, self).__init__()
        self.shift = shift
        self.border = border
        self.exp_coef = T.cast(exp_coef, dtype=theano.config.floatX)
        self.time_term = time_term
    
    def output_func(self, input):
        # input: [n_batch, T]       e.g., [1, 5, 10, 20, -MAX]
        # output: [n_batch, border+T+shift+border] 
        #   e.g., [0*border, e^-19, e^-15, e^-10, e^0, e^0* shift times, 0, 0*border]
        n_input, t_input = input.shape
        all_zeros = T.zeros((n_input, t_input+self.shift), 
                           dtype=theano.config.floatX)
        if self.time_term == 'delta':
            input_diff = input[T.arange(n_input).reshape((-1, 1)), 
                         T.arange(t_input-1).reshape((1, -1))] - \
                      input[T.arange(n_input).reshape((-1, 1)), 
                         T.arange(1, t_input).reshape((1, -1))]
        elif self.time_term == 'max':
            input_diff = input[T.arange(n_input).reshape((-1, 1)), 
                         T.arange(t_input-1).reshape((1, -1))] - \
                      T.max(input, axis=1, keepdims=True)
        else:
            assert False, 'time_diff is of wrong setting [{}]'.format(
                self.time_term)
        input_diff_pad = T.zeros((n_input, self.shift+1), dtype=theano.config.floatX)
        input_diff = T.concatenate((input_diff, input_diff_pad), axis=1)
        input_diff = T.exp(self.exp_coef * input_diff)
        input_diff = T.cast(input_diff, dtype=theano.config.floatX)
        # this is, e.g., [e^-19, e^-15, e^-10, e^0, e^0, e^0* shift times]
        input_cond_pad = T.ones((n_input, self.shift))
        input_cond = input >= 0.
        input_cond = T.concatenate((input_cond_pad, input_cond), axis=1)
        # this is, e.g., [true* shift times, true, true, true, true, false]        
        this_out = T.switch(input_cond, input_diff, all_zeros)
        # this is, e.g., [e^-19, e^-15, e^-10, e^0, e^0* shift times, 0]
        if self.border > 0:
            out_border = T.zeros((n_input, self.border), dtype=theano.config.floatX)
            this_out = T.concatenate([out_border, this_out, out_border], axis=1)
        # this is, e.g., [0*border, e^-19, e^-15, e^-10, e^0, e^0* shift times, 0, 0*border]
        return this_out
...
class LookupTableFast(Layer):
    ...
    def __init__(self, W=None, pad=None, borrow=True):
      super(LookupTableFast, self).__init__()
      self.pad = pad
      self.W = theano.shared(value=W, name='W_emb', borrow=borrow)
      self.weights = [self.W]
...
class EmbeddingInput(Layer):
    ...
    def output_func(self, input):
        '''
        Input: N * t * d
        Output: N * 1 * [padded] t * d        
        '''
        pad_matrix = T.zeros((input.shape[0], 1, self.pad, input.shape[2]))
        out_4d = input.dimshuffle([0, 'x', 1, 2])
        out = T.concatenate([pad_matrix, out_4d, pad_matrix], axis=2)
        return out        
...
class LookupTableFastWithTime(Layer):
    """ Basic linear transformation layer (W.X + b).
    Padding is used to force conv2d with valid mode behave as working in full mode."""
    def __init__(self, W=None, pad=None, exp_coef=0.01, time_term='delta', 
                 borrow=True):
      super(LookupTableFastWithTime, self).__init__()
      self.pad = pad
      self.W = theano.shared(value=W, name='W_emb', borrow=borrow)
      self.weights = [self.W]
      self.exp_coef = T.cast(exp_coef, dtype=theano.config.floatX)
    ...
    def output_func(self, input):
      x, ts = input[0], input[1]
      out = self.W[x.flatten()].reshape((x.shape[0], 1, x.shape[1], self.W.shape[1]))
      if self.time_term == 'delta':          
          ts_diff = ts[T.arange(ts.shape[0]).reshape((-1, 1)), T.arange(ts.shape[1]-1).reshape((1, -1))] - ts[T.arange(ts.shape[0]).reshape((-1, 1)), T.arange(1, ts.shape[1]).reshape((1, -1))]
          ts_pad = T.zeros((ts.shape[0], 1))
          ts_diff = T.concatenate((ts_pad, ts_diff), axis=1)
      elif self.time_term == 'max':
          ts_diff = ts - T.max(ts, axis=1, keepdims=True)
      else:
          assert False, 'ts_diff is of wrong setting [{}]'.format(self.time_term)
      ts_diff = T.cast(ts_diff, dtype=theano.config.floatX)
      ts_out = T.exp(self.exp_coef * ts_diff)
      out = out * ts_out.dimshuffle(0, 'x', 1, 'x')
      if self.pad:
        pad_matrix = T.zeros((out.shape[0], out.shape[1], self.pad, out.shape[3]), dtype=theano.config.floatX)
        out = T.concatenate([pad_matrix, out, pad_matrix], axis=2)
      return out
...
class LookupTableFastStaticWithTime(Layer):
    """ Basic linear transformation layer (W.X + b).
    Padding is used to force conv2d with valid mode behave as working in full mode."""
    def __init__(self, W=None, pad=None, exp_coef=0.01, time_term='delta'):
      super(LookupTableFastStaticWithTime, self).__init__()
      self.pad = pad
      self.W = theano.shared(value=W, name='W_emb', borrow=True)
      self.exp_coef = T.cast(exp_coef, dtype=theano.config.floatX)
      self.time_term = time_term
...
class ParallelQA(FeedForwardNet):
...
class NonLinearityLayer(Layer):
  ...
  def output_func(self, input):
    ...
    return (self.activation(input[0] + self.b.dimshuffle('x', 0, 'x', 'x')), self.activation(input[1] + self.b.dimshuffle('x', 0, 'x', 'x')))
...
class KMaxPoolLayer(Layer):
  ...
  def output_func(self, input):
    ...
    if self.k_max == 1:
      ...
      return (conv1d.max_pooling(input[0]),
            conv1d.max_pooling(input[1]))
    return conv1d.k_max_pooling(input, self.k_max)
    return (conv1d.k_max_pooling(input[0], self.k_max),
            conv1d.k_max_pooling(input[1], self.k_max))
...
class Conv2dLayer(ConvolutionLayer):
  ...
  def output_func(self, input):
    ...
    return (conv.conv2d(input[0], self.W, border_mode='valid',
                       filter_shape=self.filter_shape,
                       image_shape=self.input_shape),
            conv.conv2d(input[1], self.W, border_mode='valid',
                       filter_shape=self.filter_shape,
                       image_shape=self.input_shape)
            )
...
class BinaryLogisticRegression(Layer):
    """binary-class Logistic Regression, modified from LogisticRegression
    """
    def __init__(self, n_in, W=None, b=None):
      if not W:
        W = build_shared_zeros((n_in,), 'W_softmax')
      if not b:
        # b = build_shared_zeros(1, 'b_softmax')
        b = theano.shared(value=numpy.cast[theano.config.floatX](0.0), name='b_softmax')
    ...
    def output_func(self, input):
        # P(Y|X) = softmax(W.X + b)
        self.p_y_given_x = T.nnet.sigmoid(self._dot(input, self.W) + self.b)
...
class PairwiseNoFeatsLayer(Layer):
  def __init__(self, q_in, a_in, activation=T.tanh):
    ...
    #W = build_shared_zeros((q_in,a_in), 'W_softmax_pairwise')
    WW=numpy.zeros((q_in,a_in))
    for i in range(q_in):
        for j in range(a_in):
            WW[i,j]=float(i+j)/float(q_in)*0.01
    W = theano.shared(value=WW, name='W_softmax_pairwise', borrow=True)
    #W = build_shared_zeros((q_in,a_in), 'W_softmax_pairwise')
  ...
  def output_func(self, input):
      ...
      dot = T.batched_dot(q, T.dot(a, 0.5*(self.W.T + self.W)))
      #dot = T.batched_dot(q, T.dot(a, self.W.T))
      #####my change
      #dot = T.batched_dot(T.dot(q, self.W.T), T.dot(a, self.W.T))
      out = T.concatenate([dot.dimshuffle(0, 'x'), q, a], axis=1)
      out = T.concatenate([dot.dimshuffle(0, 'x')], axis=1)
      return out
