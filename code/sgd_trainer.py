...
class MiniBatchIteratorConstantBatchSizeSubSamping(object):
    """ Basic mini-batch iterator """
    def __init__(self, rng, datasets, batch_size=100, randomize=False, subsamping=1.0):
        self.rng = rng
        self.batch_size = batch_size
        self.n_samples = int(datasets[0].shape[0] * subsamping)
        padded_datasets = []
        for d in datasets:
          pad_size = batch_size - self.n_samples % batch_size
          pad = d[:pad_size]
          # print 'd.shape, pad', d.shape, pad.shape
          padded_dataset = numpy.concatenate([d[:self.n_samples], pad])
  ...
  def fit(self, x_train, y_train, x_dev=None, y_dev=None, batch_size=100):
    ...
    while epoch < n_epochs:
        ...
        for i, (x, y) in enumerate(train_set_iterator, 1):
            ...
            print '{0} in {1}'.format(i, len(train_set_iterator))
